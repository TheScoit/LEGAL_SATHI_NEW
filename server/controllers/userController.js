...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password !== confirmPassword) {
    return next(
      new ErrorHandler("Your password and Confirm Password donot match", 400)
    );
  }
  ...
  if (!isPasswordMatched) {
    return next(
      new ErrorHandler("Password is wrong ,Please Enter correct password", 400)
    );
  }
  if (role !== user.role) {
...
export const getAllAttorneys = catchAsyncErrors(async (req, res, next) => {
  const Attorneys = await User.find({ role: "Attorney" }).select(
    "firstName lastName email phone dob gender attorneyDepartment AttorneyAvatar"
  );
  res.status(200).json({
    success: true,
    Attorneys,
...
export const getAttorneyById = async (req, res) => {
  try {
    const attorney = await User.findOne({ _id: req.params.id, role: "Attorney" });
    if (!attorney) {
      return res.status(404).json({ success: false, message: "Attorney not found" });
    }
    res.status(200).json({ success: true, attorney });
  } catch (error) {
    console.error(error);
